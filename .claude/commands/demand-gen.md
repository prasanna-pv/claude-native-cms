...
**Section library** (each may set `bg: "plain" | "white" | "tint" | "dark"`). This is the **refined, agent-led template**, blended toward the hand-crafted **Fixify** reference: **dark/light section alternation** (dark hero with a purple radial glow, alternating dark and light bands down the page for a bold rhythm), Instrument Serif display + Manrope body + **IBM Plex Mono eyebrows**, a **dual accent** (brand color = primary; fixed Petavue purple `#6F57FF` = the "Petavue voice" on agent cards). Model the page on `data/attio.page.json` (the current worked example) and the live winners (`go.petavue.com/l`). Set `bg` per section to alternate dark↔light (e.g. hero `dark` → homework `white` → statement `dark` → capabilities `white` → agentLoop `tint` → recommendations `dark` → comparison `white` → defensibility `dark` → integrations `white` → pilot `dark` → cta `white`); `pilot` and the legacy 2-col comparison force navy regardless.
...
`hero(+card+chips)` → `homework` → `statement` (the mirror) → `capabilities` → `agentLoop` → `recommendations` → `comparison` (table) → `defensibility` → `integrations` → `pilot` → `cta`. **`calculator` is optional** — drop it from the default arc; add it only when it earns its place (see above). Vary section mix, emphasis, and copy per company so no two read identically — but keep the hero→homework→mirror opening and the pilot→cta close, and set `bg` to alternate dark↔light down the page.
