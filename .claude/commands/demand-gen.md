...
**Section library** (each may set `bg: "plain" | "white" | "tint" | "dark"`). This is the **refined, agent-led template**, modeled on the hand-crafted **Avoma** + **Plain** references: a **LIGHT-FIRST rhythm** (the page opens light and closes light; `dark` is used for **at most ~2 sections, spaced far apart**, as emphasis — never the hero — everything else white/tint), **heavy Manrope 800 display headlines + Manrope body + Manrope 700 uppercase eyebrows, with Instrument Serif reserved as an ITALIC ACCENT** — mark one or two emphasis words per headline `*like this*` in the spec and the renderer sets them in serif italic (the statement pull-quote is fully serif italic). A **dual accent** (brand color = primary; fixed Petavue purple `#6F57FF` = the "Petavue voice" on agent cards). Model the page on `data/attio.page.json` (the current worked example) and the live winners (`go.petavue.com/l`). Set `bg` **light-first** — the proven map (from `data/attio.page.json`): hero `plain` → homework `white` → statement **`dark`** → capabilities `white` → agentLoop `tint` → recommendations `white` → comparison `tint` → defensibility `white` → integrations `white` → pilot **`dark`** → booking `white`. That's **exactly two** dark bands (the mirror statement + the pilot), spaced far apart; the hero and the close stay light. `pilot` and the legacy 2-col comparison force navy regardless (so if you keep the default `pilot` as one of your two darks, don't add a third elsewhere).
...
- `capabilities` `{eyebrow, heading, sub?, items:[{icon?,title,body,outcome?}]}` — 4–6 **numbered** cards (auto 01, 02…), each tied to a number the team already owns. `outcome?` renders an accent **outcome pill** under the card — a short quantified/impact phrase (e.g. `2.6× paid-conv on vertical intent`, `$180K recoverable ARR / quarter`). Add one to most items.
...
- `recommendations` `{eyebrow, heading, body, cards:[card], note?}` where `card = {app?, time?, tag, title, found?, why?, body?, metrics:[{v,k}], move, impact, actions[]}` — 1–2 rich Slack-style agent recommendation mocks. Prefer the explicit **`found` ("What I found:") + `why` ("Why it matters:")** paragraph pair over a single `body`. **Mark them illustrative** in `note`. (Rendered with the Petavue-purple avatar/app + a purple recommendation band.)
- `comparison` **(prefer the table)** `{eyebrow, heading, sub?, rows:[{capability, dashboards, petavue}], themLabel?, usLabel?}` — a 3-column table (**Capability | Dashboards | Petavue agents**), ~6 rows, red ✗ on `dashboards`, green ✓ on `petavue`. **Backward-compat:** the older 2-column `{them:{tag,body}, us:{tag,body,points[]}}` shape still renders (dark navy). Comparison requires **either** `rows` **or** `them`+`us`.
- `calculator` **(OPTIONAL — not in the default arc)** `{eyebrow, heading, body, icp?, fit?, conv?, acv?, note?}` — interactive missed-pipeline sliders. Include **only** when a quantified "money slipping past the click" beat genuinely helps the story (and the target's economics are known); otherwise omit it. Defaults are illustrative.
...
- `cta` `{heading, body, ctaLabel}` — a punchy closing block with a single button.
- `booking` `{eyebrow, heading, body?, cal?}` — **the preferred closer for these hyper-personalized pages.** Renders a heading/subhead over a **live cal.com calendar embedded inline** (no bounce to a new tab), themed with the page's brandColor so the prospect books inside their own brand's page. `cal` = the cal.com link slug, **default `prasanna`**; prefer the full event slug when known (e.g. `prasanna/petavue`) so the embed lands directly on the meeting type instead of following a username redirect. Self-contained (its loader script ships inside the section) with a `<noscript>` link fallback. The embed loads `app.cal.com` JS at runtime, so it renders on the **deployed** page but is blank in a local `--shot` screenshot (no network) — that's expected, not a bug.
...
`hero(+card+chips)` → `homework` → `statement` (the mirror) → `capabilities` → `agentLoop` → `recommendations` → `comparison` (table) → `defensibility` → `integrations` → `pilot` → `booking` (embedded calendar; or a plain `cta`). **`calculator` is optional** — drop it from the default arc; add it only when it earns its place (see above). Vary section mix, emphasis, and copy per company so no two read identically — but keep the hero→homework→mirror opening and the pilot→booking/cta close, and keep the `bg` **light-first** (light hero, ≤2 dark bands as emphasis, light close — see the bg map above). **Prefer `booking` over `cta` as the closer** — a live in-page calendar converts better on a page this tailored; the closing copy from a `cta` (heading + body) carries straight into the `booking` header.
